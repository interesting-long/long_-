   ...
   if(!facial){
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				OLED_ShowChar(x+(sizey/2+m)*t,y,' ',sizey,0);
				continue;
			}else enshow=1;
		}
	 	OLED_ShowChar(x+(sizey/2+m)*t,y,temp+'0',sizey,0);
	}
}
     else if(facial){
         for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				OLED_ShowChar(x+(sizey/2+m)*t,y,' ',sizey,1);
				continue;
			}else enshow=1;
		}
	 	OLED_ShowChar(x+(sizey/2+m)*t,y,temp+'0',sizey,1);
	}
}
}
//显示一个字符号串
void OLED_ShowString(u8 x,u8 y,u8 *chr,u8 sizey,unsigned char facial)
{
 ...
 if(!facial){
	while (chr[j]!='\0')
	{		
		OLED_ShowChar(x,y,chr[j++],sizey,0);
		if(sizey==8)x+=6;
		else x+=sizey/2;
	}
}
	...
	while (chr[j]!='\0')
	{		
		OLED_ShowChar(x,y,chr[j++],sizey,1);
		if(sizey==8)x+=6;
		else x+=sizey/2;
	}
}
