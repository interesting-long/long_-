...
#define OLED_SCL_PIN  P63		
#define OLED_SDA_PIN  P62
#define OLED_REST_PIN P61		//液晶复位引脚定义
#define OLED_DC_PIN   P64   	//液晶命令位引脚定义
//#define OLED_CS_PIN   P51   	//定义SPI_CS引脚
...
extern uint8 oled_buffer[8][128];  // OLED显示缓冲区 (8页 x 128列)
...
void oled_clear_buffer_line(uint8 page);  // 清空缓冲区的指定行
void oled_update_line_spi(uint8 page);    // 更新屏幕的指定行
void oled_reverse_buffer(void);              // 整个缓冲区反显
void oled_reverse_buffer_line(uint8 page);   // 指定行缓冲区反显
void oled_reverse_line_immediate(uint8 page); // 指定行反显并立即更新
